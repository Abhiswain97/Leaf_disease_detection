...
class Imbalance:
    ...
    @staticmethod
    def _all_knn(X, y):
        return under_sampling.AllKNN().fit(X=X, y=y)

    @staticmethod
    def _condensed_nearest_neighbour(X, y):
        return under_sampling.CondensedNearestNeighbour().fit(X=X, y=y)

    @staticmethod
    def _edited_nearest_neighbours(X, y):
        return under_sampling.EditedNearestNeighbours().fit(X=X, y=y)

    @staticmethod
    def _repeated_edited_nearest_neighbours(X, y):
        return under_sampling.RepeatedEditedNearestNeighbours().fit(X=X, y=y)

    @staticmethod
    def _random_under_sampler(X, y):
        return under_sampling.RandomUnderSampler().fit(X=X, y=y)
