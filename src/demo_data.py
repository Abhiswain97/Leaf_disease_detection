    ...
    def segment_and_store(self):
        ...
        for i, folder in enumerate(os.listdir(self.path)):
            ...
            for file in tqdm(os.listdir(os.path.join(self.path, folder))):
                ...
                if os.path.exists(
                    os.path.join(self.path, seg_folder[i] + f"\\(disease){file}")
                ) or folder.endswith("seg"):
                    logging.info("[Segmented Image Already Exists] in " + f"{folder}")
                else:
                    print(os.path.join(self.path, folder + f"\\{file}"))
                    image = cv2.imread(
                        os.path.join(self.path, folder + f"\\{file}")
                    ).astype("uint8")
                    image = cv2.resize(image, (512, 512))
                    # image = image.reshape((-1, 3))

                    hsv_image = self.rgb_to_hsv(image).reshape(
                        (-1, 3)
                    )  # TODO: Handle image conversion to hsv errors
                    kmeans = KMeans(n_clusters=4, random_state=0).fit(
                        hsv_image
                    )  # TODO: KMeans Elbow method and #
                    # Dendrogram
                    w, h, d = tuple(image.shape)
                    self.recreate_image(kmeans.cluster_centers_, kmeans.labels_, w, h)
                    print(kmeans.labels_)
                    segmented_image = self.seperate_cluster(
                        hsv_image.reshape(image.shape), kmeans.labels_
                    )
                    # cv2.imwrite(os.path.join(self.path, seg_folder[i] + f'\\(disease){file}'), segmented_image)
                    logging.info(
                        "[SAVED] {0}".format(
                            os.path.join(
                                self.path, seg_folder[i] + f"\\(disease){file}"
                            )
                        )
                    )
